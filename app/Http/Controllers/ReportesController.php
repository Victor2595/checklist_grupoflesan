...
class ReportesController extends Controller
{
	public function reportWeek(){
		try {
			...
			$tabla_visita = DB::select("select * from(select b.clbod_semana,b.clbod_ano,b.clbod_obra_id,p.nombre_proyecto,COALESCE(a.estado,'no_existe') as almacen,COALESCE(v.estado,'no_existe') as visita,b.clbod_ano||'_'||b.clbod_semana fecha from abastecimiento.clbod b full join (select clbod_obra_id,clbod_cumplimiento,clbod_semana,case when clbod_validate_date is null then 'pendiente' when clbod_validate_date is not null then 'aprobado' else 'no_existe' end estado from abastecimiento.clbod where clbod.clbod_tipo = 1)a on a.clbod_obra_id = b.clbod_obra_id and a.clbod_semana = b.clbod_semana full join (select clbod_obra_id,clbod_cumplimiento,clbod_semana , case	when clbod_validate_date is null then 'pendiente' when clbod_validate_date is not null then 'aprobado' else 'no_existe' end estado  from abastecimiento.clbod where clbod.clbod_tipo = 2 )v on v.clbod_obra_id = b.clbod_obra_id and v.clbod_semana = b.clbod_semana inner join ggo.ggo_proyecto p on trim(p.cod_proyecto) = b.clbod_obra_id GROUP BY b.clbod_semana,b.clbod_ano,b.clbod_obra_id,a.estado,v.estado,p.nombre_proyecto)a where ((case when a.almacen = 'aprobado' then 1 else 0 end ) + (case when a.visita = 'aprobado' then 1 else 0 end )) < 2 ORDER BY a.clbod_obra_id,a.clbod_semana ASC,clbod_ano ASC");

			$segui_visit = DB::select("select u.name,clbod_semana||'-'||clbod_ano as fecha,count(clbod_semana)numero from abastecimiento.clbod b inner join seguridadapp.aplicacion_usuario u on u.id_aplicacion_usuario = CAST(b.clbod_create_user as integer) inner join ggo.ggo_proyecto p on trim(p.cod_proyecto) = b.clbod_obra_id where b.clbod_tipo = 2 and b.clbod_ano = $año group by u.name,clbod_semana,clbod_ano order by b.clbod_semana asc");

			$segui_visit_acu = DB::select("select u.name,count(clbod_semana)numero from abastecimiento.clbod b inner join seguridadapp.aplicacion_usuario u on u.id_aplicacion_usuario = CAST(b.clbod_create_user as integer) inner join ggo.ggo_proyecto p on trim(p.cod_proyecto) = b.clbod_obra_id where b.clbod_tipo = 2  and b.clbod_ano = $año group by u.name order by u.name");
