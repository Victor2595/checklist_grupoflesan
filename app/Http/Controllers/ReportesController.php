...
class ReportesController extends Controller
{
	public function reportWeek(){
		...
		$tabla_visita = DB::select("select distinct b.clbod_semana,b.clbod_ano,b.clbod_obra_id,p.nombre_proyecto,a.clbod_cumplimiento as almacen,v.clbod_cumplimiento as visita,b.clbod_ano||'_'||b.clbod_semana fecha from abastecimiento.clbod b full join (select clbod_obra_id,clbod_cumplimiento,clbod_semana from abastecimiento.clbod where clbod.clbod_tipo = 1 and clbod_validate_date is not null)a on a.clbod_obra_id = b.clbod_obra_id and a.clbod_semana = b.clbod_semana full join (select  clbod_obra_id,clbod_cumplimiento,clbod_semana from abastecimiento.clbod where clbod.clbod_tipo = 2 and clbod_validate_date is not null)v on v.clbod_obra_id = b.clbod_obra_id and v.clbod_semana = b.clbod_semana inner join ggo.ggo_proyecto p on trim(p.cod_proyecto) = b.clbod_obra_id where a.clbod_cumplimiento is null or v.clbod_cumplimiento is null GROUP BY b.clbod_ano, b.clbod_semana, b.clbod_obra_id, p.nombre_proyecto, a.clbod_cumplimiento, v.clbod_cumplimiento,b.clbod_validate_date ORDER BY b.clbod_obra_id,b.clbod_semana DESC,clbod_ano DESC");
